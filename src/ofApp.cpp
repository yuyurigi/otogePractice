...
void ofApp::mousePressed(int x, int y, int button){
    ...
    if(bVideoBox){
        for (int i = 0; i < videos.size(); i++) {
            ...
            //動画タイトルのテキストを押したとき
            if (videoBoxR.x+80 <= mouseX && mouseX <= videoBoxR.x+videoBoxR.getWidth()-padding && videoBoxR.y+padding*1.5+videoNameR[i].y <= mouseY && mouseY <= videoBoxR.y+padding*1.5+videoNameR[i].y+videoNameR[0].getHeight()) {
